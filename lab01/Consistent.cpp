...
int main() {
    ...
    */
    ...
    //     for (int m = 1; m < M; ++m) {
    //         if (m < M-1)
    //             u[(k+1)*M + m] = f_arr[k*M + m] * tau + (tau / (2*h) + tau*tau / (2*h*h)) * u[k*M + m-1] + (tau*tau / (2*h*h) - tau / (2*h)) * u[k*M + m+1] + (1 - tau*tau / (h*h)) * u[k*M + m];
    //         else
    //             u[(k+1)*M + m] = f_arr[k*M + m] * tau + (h - tau)/h * u[k*M + m] + tau/h * u[k*M + m-1];
    //     }
    // }
    ...
    //! Можно написать функцию для получение индекса соответствующего одномерного массива по индексам двумерного
    // for (int m = 1; m < M; ++m) {
